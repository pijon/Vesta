...
| -------- | ------------------------------------------- | -------------------- | ----------------------------------------- |
| NAMING   | `components/RecipeCard.tsx:10-15`         | PascalCase components| `export const RecipeCard: React.FC = ()`  |
| STATE    | `App.tsx:20-40`                             | Global state props   | `const [recipes, setRecipes] = useState`  |
| LOGGING  | `utils/logger.ts:1-10`                      | Console wrappers     | `console.error("Error:", error)`          |
| STYLING  | `index.css:50-80`                           | Semantic classes     | `.btn-primary { @apply bg-emerald-500 }`  |
| TYPES    | `types.ts:1-20`                             | Shared interfaces    | `export interface Recipe { id: string }`  |
...
**OUTPUT_PATH**: `.agent/plans/{kebab-case-feature-name}.plan.md`

Create directory if needed: `mkdir -p .agent/plans`
...
|----------|------|-------|---------------|
| P0 | `.agent/knowledge/design_system.md` | all | **MANDATORY** - Complete Digital Hearth design rules (colors, typography, components, dark mode) |
| P0 | `types.ts` | 1-30 | Types to IMPORT |
| P1 | `App.tsx` | 10-50 | State management pattern |
| P2 | `services/storageService.ts` | all | Data persistence pattern |
...
| -------------------------------- | ------ | ---------------------------------------- |
| `components/NewFeature.tsx`      | CREATE | UI Component                             |
| `services/featureService.ts`     | CREATE | Logic extraction (if needed)             |
| `types.ts`                       | UPDATE | Add new definitions                      |
| `App.tsx`                        | UPDATE | Integrate component/state                |
| `index.css`                      | UPDATE | Add global styles (if needed)            |
...
- **ACTION**: ADD interface definitions
- **IMPLEMENT**: {specific interfaces}
- **MIRROR**: `types.ts:XX-YY`
- **VALIDATE**: `npx tsc --noEmit` - types must compile

### Task 2: UPDATE Logic in `services/storageService.ts`

- **ACTION**: ADD persistence methods
- **IMPLEMENT**: save/load logic for new data
- **MIRROR**: `services/storageService.ts:XX-YY`
- **INVALIDATE**: Ensure no data loss risk
- **VALIDATE**: Logic check / Manual verify

### Task 3: CREATE Component `components/NewFeature.tsx`

- **ACTION**: CREATE UI component
- **IMPLEMENT**: Layout, Styling, Interactions
- **STYLE**: Use semantic colors (`bg-emerald-500`, `text-slate-900`)
- **MIRROR**: `components/RecipeCard.tsx` (or similar)
- **VALIDATE**: `npm run dev` -> Check rendering

### Task 4: INTEGRATE in `App.tsx`

- **ACTION**: WIRE up state and component
- **IMPLEMENT**: Add state, handlers, render component
- **VALIDATE**: `npm run dev` -> Check full flow
...
| ---------------------------------------- | -------------------------- | ---------------- |
| **Design System Compliance**             | Manual Review              | All UI adheres to `.agent/knowledge/design_system.md` (semantic colors, serif headings, dark mode) |
| **Visual Polish**                        | Browser (Manual)           | "Rich Aesthetics" met, aligns with `ui_colors.md` |
| **Mobile Responsiveness**                | Browser (Resize)           | Layout adjusts, no overflow |
| **State Persistence**                    | Browser (Reload)           | Data persists after reload |
...
**EXPECT**: Build completes successfully (dist/ generated)

### Level 3: BROWSER_VALIDATION

Use Browser MCP or Manual Verification to verify:

- [ ] UI renders correctly (Desktop & Mobile)
- [ ] User flows work end-to-end
- [ ] Animations feel smooth
- [ ] "Rich Aesthetics" achieved
...
- [ ] Level 1 & 2 validation commands pass with exit 0
- [ ] Visual verification confirms "Rich Aesthetics" (Glassmorphism, correct colors)
- [ ] Mobile responsive behavior verified
- [ ] No regressions in core flows (App not crashing)
...
<output>
**OUTPUT_FILE**: `.agent/plans/{kebab-case-feature-name}.plan.md`
...
**File**: `.agent/plans/{feature-name}.plan.md`
...
**Next Step**: To execute, run: `/prp-implement .agent/plans/{feature-name}.plan.md`
````
