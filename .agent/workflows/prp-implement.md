...
- **Mandatory Reading** - MUST read `.agent/knowledge/design_system.md` before implementation
...
- `{runner} run type-check` (TS projects)
- `{runner} tsc --noEmit` (TS alternative)
...
```
Task 1: CREATE components/RecipeCard.tsx ✅
Task 2: CREATE services/recipeService.ts ✅
...
**Path**: `.agent/reports/{plan-name}-report.md`
...
| #   | Task               | File       | Status |
| --- | ------------------ | ---------- | ------ |
| 1   | {task description} | `components/X.tsx` | ✅     |
| 2   | {task description} | `services/Y.ts` | ✅     |
...
| File       | Action | Lines     |
| ---------- | ------ | --------- |
| `components/X.tsx` | CREATE | +{N}      |
| `services/Y.ts` | UPDATE | +{N}/-{M} |
...
| --------------- | ------------------------ |
| `utils/x.test.ts` | {list of test functions} |
...
```bash
mkdir -p .agent/plans/completed
mv $ARGUMENTS .agent/plans/completed/
```

**PHASE_5_CHECKPOINT:**

- [ ] Report created at `.agent/reports/`
...
- Report: `.agent/reports/{name}-report.md`
- Plan archived to: `.agent/plans/completed/`
