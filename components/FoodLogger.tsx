...
export const FoodLogger: React.FC<FoodLoggerProps> = ({ currentLog, onAddItems, onUpdateWeight, userStats }) => {
  ...
  return (
    <div className="space-y-6 pb-20">
      <header>
        <h2 className="text-2xl font-bold text-slate-800 mb-2">Daily Log</h2>
        <p className="text-slate-500">Track what you eat to stay on target.</p>
      </header>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <div className="flex gap-3">
            <input 
                type="text"
                className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
            ...
            <button
                ...
                className={`px-6 rounded-xl font-semibold text-white transition-all ${
                    isAnalyzing || !input.trim() ? 'bg-slate-300' : 'bg-emerald-600 hover:bg-emerald-700'
                }`}
            >
                {isAnalyzing ? '...' : 'Add'}
            </button>
        </div>
        <p className="text-xs text-slate-400 mt-2 ml-1">AI will estimate calories automatically.</p>
      </div>

       {/* Weight Update */}
       <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
            <div>
                 <h3 className="font-semibold text-slate-800">Current Weight (kg)</h3>
                 <p className="text-xs text-slate-400">Update daily for best results</p>
            </div>
            <div className="flex gap-2">
                <input 
                    ...
                    className="w-24 p-2 bg-slate-50 border border-slate-200 rounded-lg text-center font-bold text-slate-700"
                />
                 <button
                    onClick={handleWeightUpdate}
                    className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 font-medium rounded-lg transition-colors text-sm"
                >
                    Save
                </button>
            </div>
