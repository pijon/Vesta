...
const TabButton: React.FC<TabButtonProps> = ({ active, onClick, children }) => (
    <button
        ...
        className={`px-6 py-3 text-sm font-bold transition-all border-b-2 ${active
            ...
            : 'border-transparent text-muted hover:text-main hover:bg-surface'
            }`}
    >
        {children}
    </button>
                        ...
                        {activeTab === 'ingredients' && (
                            <div className="space-y-4 animate-fade-in">
                                <h3 className="flex items-center gap-2 text-xl font-bold text-main font-serif mb-4">
                                    <span className="bg-primary-light text-primary p-1.5 rounded-lg"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 13.87A8 8 0 1 1 6 10a8 8 0 0 1 8-4 8 8 0 0 1 8 4"></path><polyline points="22 6 22 17 6 17 6 6"></polyline></svg></span>
                                    Shopping List
                                </h3>
                                    ...
                                    <div className="space-y-0 relative before:absolute before:inset-0 before:ml-4 before:-translate-x-px before:h-full before:w-0.5 before:bg-border before:content-['']">
                                        {recipe.instructions.map((step, i) => (
                                            <div key={i} className="relative pl-12 pb-8 group">
                                                <span className="absolute left-0 top-0 w-8 h-8 rounded-full bg-background text-muted text-sm font-bold flex items-center justify-center ring-4 ring-surface group-hover:bg-primary group-hover:text-primary-foreground transition-colors border border-border">{i + 1}</span>
